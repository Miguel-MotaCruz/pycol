...
def process_dataset(Partitions_folder, dataset_name, n_splits, reps, partition_strategy, metrics, Metrics_folder):
    dataset_name = dataset_name.split(".")[0]
    print(dataset_name, n_splits, partition_strategy)
    for partition_iteration in range(1, reps+1):
        for i in range(1, n_splits+1):
            train_split_path = os.path.join( Partitions_folder ,dataset_name+"-"+partition_strategy, dataset_name+"-"+partition_strategy+"-"+str(partition_iteration)+"-"+str(n_splits)+"-"+str(i)+"tra"+".arff")
            test_split_path = os.path.join( Partitions_folder ,dataset_name+"-"+partition_strategy, dataset_name+"-"+partition_strategy+"-"+str(partition_iteration)+"-"+str(n_splits)+"-"+str(i)+"tst"+".arff")
            ...
            for metric in metrics:
                ...
                with open(os.path.join(Metrics_folder, f'{n_splits}_folds_{partition_strategy}' ,f'{dataset_name}_{n_splits}_{partition_strategy}_{metric[0]}.csv'), 'a', newline='') as file:
...
def delete_csv_files(Metrics_folder, dataset_name, n_splits, partition_strategy, metrics):
    for metric in metrics:
        file_path =os.path.join(Metrics_folder, f'{n_splits}_folds_{partition_strategy}' ,f'{dataset_name}_{n_splits}_{partition_strategy}_{metric[0]}.csv')
        # print(file_path)
        if os.path.exists(file_path):
            os.remove(file_path)

def process(args):
    Partitions_folder, Metrics_folder, dataset_name, n_splits, reps, partition_strategy, metrics = args
    delete_csv_files(Metrics_folder, dataset_name.split(".")[0], n_splits, partition_strategy, metrics)
    process_dataset(Partitions_folder, dataset_name, n_splits, reps, partition_strategy=partition_strategy, metrics=metrics, Metrics_folder=Metrics_folder)

def main(partition_strategies, feature_metrics, structural_metrics, instance_metrics, multiresolution_metrics, partitioned_files_json, Partitions_folder ,Metrics_folder, n_splits, reps):
    # n_splits = 5
    # reps = 50
    ...
    for partition_strategy in partition_strategies:
            create_directory(os.path.join(Metrics_folder, f'{n_splits}_folds_{partition_strategy}'))
    ...
    for dataset_name in partitioned_files[Partitions_folder]:
        for partition_strategy in partition_strategies[:]:
            ...
            # process(args)
            args_list.append((Partitions_folder,Metrics_folder ,dataset_name, n_splits, reps, partition_strategy, all_metrics))
